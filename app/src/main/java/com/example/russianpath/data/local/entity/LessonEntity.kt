...
/**
 * Урок — контейнер для вопросов и теории.
 * Связан с TopicEntity и LearningObjectiveEntity.
 */
@Entity(
    tableName = "lessons",
    foreignKeys = [
    ...
    indices = [
        Index(
            ...
            name = "idx_lessons_topic_sort"
        ),
        Index(
            value = ["topic_id", "lesson_type", "sort_order"],
            name = "idx_lessons_topic_type_sort"
        ),
        Index(
            value = ["external_id"],
            name = "idx_lessons_external_id",
            unique = true
        ),
        Index(
            value = ["topic_id"],
            name = "idx_lessons_topic_id"
        ),
        Index(
            value = ["primary_objective_id"],
            name = "idx_lessons_objective_id"
        ),
        Index(
            value = ["is_active"],
...
data class LessonEntity(
    ...
    @ColumnInfo(name = "primary_objective_id", defaultValue = "")
