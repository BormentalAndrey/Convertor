...
/**
 * Разделы внутри класса (например, "Фонетика", "Морфология", "Синтаксис").
 * Связаны с GradeEntity через gradeId.
 */
@Entity(
    tableName = "sections",
    foreignKeys = [
    ...
    indices = [
        Index(
            ...
            name = "idx_sections_grade_sort"
        ),
        Index(
            value = ["external_id"],
            name = "idx_sections_external_id",
            unique = true
        ),
        Index(
            value = ["grade_id"],
            name = "idx_sections_grade_id"
        )
    ]
)
data class SectionEntity(
    ...
    @ColumnInfo(name = "color_hex", defaultValue = "#FF6200EE")
