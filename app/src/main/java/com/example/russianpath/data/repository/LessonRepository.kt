...
class LessonRepository @Inject constructor(
    ...
    fun observeLessonsByTopicAndType(topicId: String, lessonType: LessonType): Flow<List<Lesson>> {
    ...
    suspend fun enrichWithProgress(lessons: List<Lesson>): List<Lesson> {
        return withContext(Dispatchers.IO) { lessons.map { enrichWithProgress(it) } }
    }
    ...
    suspend fun getQuestionsByDifficultyRange(minDifficulty: Int, maxDifficulty: Int, limit: Int = 20): List<Question> {
        return withContext(Dispatchers.IO) {
            questionDao.getByDifficultyRange(minDifficulty, maxDifficulty, limit).map { it.toDomainModel() }
        }
    }

    suspend fun getQuestionCountByLesson(lessonId: String): Int {
        return withContext(Dispatchers.IO) { questionDao.countByLesson(lessonId) }
    }
    ...
    suspend fun getLessonWithQuestions(lessonId: String): LessonWithQuestions? {
        return withContext(Dispatchers.IO) {
            ...
            val questions = questionDao.getAllByLesson(lessonId).map { it.toDomainModel() }
            LessonWithQuestions(lesson = enrichedLesson, questions = questions)
        }
    }

    suspend fun getLessonsWithQuestionsByTopic(topicId: String): List<LessonWithQuestions> {
        return withContext(Dispatchers.IO) {
            lessonDao.getAllByTopic(topicId).map { entity ->
                val lesson = entity.toDomainModel()
                val enrichedLesson = enrichWithProgress(lesson)
                val questions = questionDao.getAllByLesson(entity.id).map { it.toDomainModel() }
                LessonWithQuestions(lesson = enrichedLesson, questions = questions)
            }
        }
    }
    ...
    suspend fun getLessonsForSpacedRepetition(limit: Int = 5): List<LessonWithQuestions> {
        return withContext(Dispatchers.IO) {
            lessonDao.getLessonsWithMistakes(limit).map { entity ->
                val lesson = entity.toDomainModel()
                val enrichedLesson = enrichWithProgress(lesson)
                val questions = questionDao.getAllByLesson(entity.id).map { it.toDomainModel() }
                LessonWithQuestions(lesson = enrichedLesson, questions = questions)
    ...
    private fun QuestionEntity.toDomainModel(): Question {
        ...
        return when (type) {
            QuestionType.SINGLE_CHOICE -> {
                val options = parseOptionsFromDataJson(dataJson)
                Question(
                    id = id, lessonId = lessonId,
                    primarySkillId = primarySkillId ?: "",
                    questionType = type, promptText = promptText,
                    promptAudioPath = promptAudioPath, promptImagePath = promptImagePath,
                    options = options, correctAnswer = correctAnswerJson,
                    acceptableAnswers = parseStringListFromJson(acceptableAnswersJson),
                    hintText = hintText, explanationText = explanationText,
                    audioPath = audioPath, ruleReference = ruleReference,
                    ruleReferenceId = ruleReferenceId, difficulty = difficulty,
                    timeLimitSeconds = timeLimitSeconds, points = points,
                    penaltyPoints = penaltyPoints, maxAttempts = maxAttempts,
            ...
            QuestionType.MULTIPLE_CHOICE -> {
                ...
                Question(
                    ...
                    acceptableAnswers = parseStringListFromJson(acceptableAnswersJson),
                    hintText = hintText, explanationText = explanationText,
                    ruleReference = ruleReference, ruleReferenceId = ruleReferenceId,
                    difficulty = difficulty, points = points, isRequired = isRequired
                )
            }
            QuestionType.TEXT_INPUT, QuestionType.FILL_IN_BLANK, QuestionType.DICTATION -> {
                Question(
                    id = id, lessonId = lessonId,
                    primarySkillId = primarySkillId ?: "",
                    questionType = type, promptText = promptText,
                    promptAudioPath = promptAudioPath, correctAnswer = correctAnswerJson,
                    acceptableAnswers = parseStringListFromJson(acceptableAnswersJson),
                    hintText = hintText, explanationText = explanationText,
                    audioPath = audioPath, ruleReference = ruleReference,
                    ruleReferenceId = ruleReferenceId, difficulty = difficulty,
                    points = points, isRequired = isRequired
                )
            }
            QuestionType.WORD_DRAG, QuestionType.SEQUENCE_ORDER -> {
                val dragData = parseDragDataFromJson(dataJson)
                Question(
                    id = id, lessonId = lessonId,
                    primarySkillId = primarySkillId ?: "",
                    questionType = type, promptText = promptText,
                    draggableWords = dragData.first, correctOrder = dragData.second,
                    correctAnswer = correctAnswerJson, hintText = hintText,
                    explanationText = explanationText, ruleReference = ruleReference,
                    ruleReferenceId = ruleReferenceId, difficulty = difficulty,
                    points = points, isRequired = isRequired
                )
            }
            QuestionType.MATCHING -> {
                ...
                Question(
                    ...
                    primarySkillId = primarySkillId ?: "",
                    questionType = type, promptText = promptText,
                    options = pairs.first, correctAnswer = correctAnswerJson,
                    hintText = hintText, explanationText = explanationText,
                    ruleReference = ruleReference, difficulty = difficulty,
                    points = points, isRequired = isRequired
                )
            }
            QuestionType.STRESS_SELECTION, QuestionType.MORPHEMIC_ANALYSIS -> {
                Question(
                    id = id, lessonId = lessonId,
                    primarySkillId = primarySkillId ?: "",
                    questionType = type, promptText = promptText,
    ...
    private fun parseOptionsFromDataJson(json: String): List<String> {
        return try { gson.fromJson(json, object : TypeToken<List<String>>() {}.type) ?: emptyList() }
    ...
    private fun parseDragDataFromJson(json: String): Pair<List<String>, List<Int>> {
        return try {
            ...
            val order = (map["correct_order"] as? List<*>)?.map { (it as? Double)?.toInt() ?: 0 } ?: emptyList()
            Pair(words, order)
